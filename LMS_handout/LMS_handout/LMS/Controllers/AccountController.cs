        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
           ...
           // Comparer<uint> cmp = (uint lhs, uint rhs)->lhs - rhs;
            ...
            var query2 =
                ...
                where d.Abbrv == SubjectAbbrev
                select d.DepartmentId).ToList();

            foreach (var entry in query2)
            {
                dID = int.Parse(entry.ToString());
            }
            ...
            else
            {
                ...
                p.UId = (uint)highest;
                p.Dob = DOB;
                p.DepartmentId = (uint)dID;
                db.Professor.Add(p);
            }
            db.SaveChanges();

            string paddedHighest = highest.ToString().PadLeft(7, '0');

            return "u" + paddedHighest;
        }
